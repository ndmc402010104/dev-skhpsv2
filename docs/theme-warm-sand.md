...
## 7. 主題切換預覽（2026-07-17 Fable 設計定稿，Sonnet 實作，✅ 已完成並 dev 驗證）
...
### 實作完成紀錄（2026-07-17 深夜）

全部五項完成並 dev 驗證通過，commits：
- skhps-backend-worker `d89901a`（theme 白名單解析＋Default 表指標）
- skhpsv2 `220a3d6`（runtime 帶 theme 參數＋快取分 key＋SKHPSThemePreview）
  `0c02313`（補 WORKER_JSON_ACTIONS 白名單漏掉 getAppDefaults/
  saveAppDefaults——真正的 bug，不是傳播延遲，別再誤判）
- 6 個暖砂 package manifest 補 themeName、新增 entry-cards-base 共用層
  （sortOrder 15，無 themeName）
- css-setting `fd408d5`：**過程中發現原設計的「per-browser 預覽」
  （localStorage）在真實網域拓樸下對 skhpsv2 沒用**——
  css-setting.skhps.jonaminz.com 跟 dev-skhps.jonaminz.com 是不同網域，
  瀏覽器儲存不共用，切了但看的網站不會變。改成「全站切換」（呼叫既有
  getAppDefaults/saveAppDefaults 寫 Default 表指標）為主、「本頁預覽」
  （localStorage）為輔，UI 上明確分開標示。也順便合併了 dev/main 上
  Codex 遺留的「CSS package 治理」commit（cb87c42，表單式 package 編輯
  工具，功能不重疊，merge 時處理了 app.js 的機械性衝突）。
- **Playwright 端到端驗證**：CSS Setting 點「全站切成 Theme40」→
